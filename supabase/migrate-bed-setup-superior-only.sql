-- Bed choice is Superior (courtyard) only; other rooms store null.
-- Run in Supabase SQL editor after deploy.

alter table public.booking_requests
  alter column bed_setup drop not null;

alter table public.booking_requests
  alter column bed_setup drop default;
...
alter table public.booking_requests
  ...
  check (bed_setup is null or bed_setup in ('double', 'twin'));

-- Clear preference on rooms that no longer offer a choice.
update public.booking_requests
set bed_setup = null
where room_id is distinct from 'courtyard';
